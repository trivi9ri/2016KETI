...
#	f10 = open("KETI.txt",'w')
	...
	for a in data.strip().split('}'):
		...
		if len(b.strip().split(' ')) == 5 :
			...
			tmp_str = b.strip().split(' ')[1] + " " + b.strip().split(' ')[4]+"\n"
#			f10.write(tmp_str)
...
def get_sensor_number():
	...
	for i in idle_data.strip().split(']'):
		 ...
		 if len(idle_uuid_detail) > 5 :
		 	...
		 	sensor_type = idle_uuid_detail[num_idx+1].split('/')[3]
		 	floor_num = idle_uuid_detail[floor_idx+1]
		 	sentence = "sensor number: %s , sensor type: %s , floor: %s\n" % (sensor_num,sensor_type,floor_num)
		 	fl3.write(sentence)
	fl3.close()
	fl2.close()
